...
void  ZTauHTauH::Configure(){
  ...
  for(int i=0; i<NCuts;i++){
    ...
    if(i==DiMuonVeto)          cut.at(DiMuonVeto)=1;
  ...
  for(int i=0; i<NCuts; i++){
    ...
    if(i==TriggerOk){
      ...
      hlabel=" Di Muon Veto  ";
      Nminus1.push_back(HConfig.GetTH1D(Name+c+"_Nminus1_DiMuonVeto_",htitle,4,-0.5,3.5,hlabel,"Events"));
      Nminus0.push_back(HConfig.GetTH1D(Name+c+"_Nminus0_DiMuonVeto_",htitle,4,-0.5,3.5,hlabel,"Events"));
    }
    ...
    /*else if(i==PairMass){
      ...
      title.at(i)="Missing transverse energy";
      hlabel="MET";
      Nminus1.push_back(HConfig.GetTH1D(Name+c+"_Nminus1_MET_",htitle,30,0,300,hlabel,"Events"));
      Nminus0.push_back(HConfig.GetTH1D(Name+c+"_Nminus0_MET_",htitle,30,0,300,hlabel,"Events"));
      }*/
  ...
  Tau1Eta=HConfig.GetTH1D(Name+"_Tau1Eta","Pseudorapidity of the first tau candidate",30,-2.3,2.3," #eta(#tau)","Events");
  Tau1Phi=HConfig.GetTH1D(Name+"_Tau1Phi","Azimuth angle of the first tau candidate",30,-(TMath::Pi()),(TMath::Pi()),"#phi(#tau)","Events");
  ...
  Tau2Phi=HConfig.GetTH1D(Name+"_Tau2Phi","Azimuth angle of the second tau candidate",30,-(TMath::Pi()),(TMath::Pi())," #phi(#tau)","Events");
  ...
  TauTauMass=HConfig.GetTH1D(Name+"_TauTauMass","Visible invariant mass of a tau pair",35,50 ,150," M(#tau#tau), GeV","Events");
  TauTauEta=HConfig.GetTH1D(Name+"_TauTauEta","Pseudorapidity of a tau pair",30,-2.3,2.3," #eta(#tau#tau)","Events");
  TauTauPhi=HConfig.GetTH1D(Name+"_TauTauPhi","Azimuth angle of a tau pair",30,-(TMath::Pi()),(TMath::Pi())," #phi(#tau#tau)","Events");

  AllJetsE=HConfig.GetTH1D(Name+"_AllJetsE","Energy of all jets",30,0,500," E(alljets), GeV","Events");
  AllJetsPT=HConfig.GetTH1D(Name+"_AllJetsPT","Transverse energy of all jets",30,0,500," ET(alljets)","Events");
  LeadingJetE=HConfig.GetTH1D(Name+"_LeadingJetE","Energy of the leading jet",30,0,300," E(j1)","Events");
  LeadingJetPT=HConfig.GetTH1D(Name+"_LeadingJetPT","Transverse energy of the leading jet",30,0,300," ET(j1), GeV","Events");
  LeadingJetEta=HConfig.GetTH1D(Name+"_LeadingJetEta","Pseudorapidity of the leading jet",30,-2.3,2.3,"#eta(j1)","Events");
  LeadingJetPhi=HConfig.GetTH1D(Name+"_LeadingJetPhi","Azimuth angle of the leading jet",30,-(TMath::Pi()),(TMath::Pi())," #phi(j1) ","Events");

  MET=HConfig.GetTH1D(Name+"_MET","Missing transverse energy",30,0 ,500," MET, GeV","Events");
  METPhi=HConfig.GetTH1D(Name+"_METPhi","Azimuth angle of the missing transverse energy",30,-(TMath::Pi()),(TMath::Pi())," #phi(MET)","Events");
...
void  ZTauHTauH::Store_ExtraDist(){
  ...
  Extradist1d.push_back(&TauTauEta);
  Extradist1d.push_back(&TauTauPhi);

  Extradist1d.push_back(&AllJetsE);
  Extradist1d.push_back(&AllJetsPT);
  Extradist1d.push_back(&LeadingJetE);
  Extradist1d.push_back(&LeadingJetPT);
  Extradist1d.push_back(&LeadingJetEta);
  Extradist1d.push_back(&LeadingJetPhi);
...
void  ZTauHTauH::doEvent(){ //  Method called on every event
  ...
  if(!HConfig.GetHisto(Ntp->isData(),id,t)){ Logger(Logger::Error) << "failed to find id  " << id  <<std::endl; return;}  //  gives a warining if list of samples in Histo.txt  and SkimSummary.log do not coincide 
  ...
  for(unsigned int iDaugther=0;   iDaugther  <  Ntp->NDaughters() ;iDaugther++ ){  // loop over all daughters in the event
    if(Ntp->isTightGoodTau(iDaugther)){
      if(Ntp->tauBaselineSelection(iDaugther,20., 2.3, 0,1)){
  ...
  for(unsigned int ipair =0; ipair < Sorted.size(); ipair++){
    ...
    if(Ntp->tauBaselineSelection(Ntp->indexDau1(Sorted.at(ipair)), 40,2.1,0,0) && Ntp->tauBaselineSelection(Ntp->indexDau2(Sorted.at(ipair)), 40,2.1,0,0)) 
      SortedPair_PassedBaseline.push_back(Sorted.at(ipair));
  }
  for(unsigned int ipair =0; ipair < SortedSS.size(); ipair++){
    if(Ntp->tauBaselineSelection(Ntp->indexDau1(SortedSS.at(ipair)), 40,2.1,0,0) && Ntp->tauBaselineSelection(Ntp->indexDau2(SortedSS.at(ipair)), 40,2.1,0,0)) 
  ...
  value.at(DiMuonVeto) = thirdLeptonCounter.size();
  pass.at(DiMuonVeto) = ( value.at(DiMuonVeto) ==  value.at(DiMuonVeto));
  ...
  if(pass.at(nGoodPairs)){
    ...
    if(Ntp->isOSCand(SortedPair_PassedBaseline.back()))    value.at(PairCharge) =1; // To Verify
    else  value.at(PairCharge) =0;
    value.at(FirstTauIsolation) = Ntp->isIsolatedTau(TauIndex_1,"Tight");
    value.at(SecondTauIsolation) = Ntp->isIsolatedTau(TauIndex_2,"Tight");

    // value.at(Met) = Ntp->MET();
 ...
 if(passAllBut(exclude_cuts_ForTauIso)){
  ...
  for(unsigned int ijet; ijet< Ntp->JetsNumber(); ijet++)
    {
      AllJetsPT.at(t).Fill((Ntp->Jet_P4(ijet)).Pt(),w);
      AllJetsE.at(t).Fill((Ntp->Jet_P4(ijet)).E(),w);
    }

  LeadingJetE.at(t).Fill((Ntp->Jet_P4(0)).E(),w);
  LeadingJetPT.at(t).Fill((Ntp->Jet_P4(0)).Pt(),w);
  LeadingJetEta.at(t).Fill((Ntp->Jet_P4(0)).Eta(),w);
  LeadingJetPhi.at(t).Fill((Ntp->Jet_P4(0)).Phi(),w);

  TauTauEta.at(t).Fill((Tau1P4+Tau2P4).Eta(),w);
  TauTauPhi.at(t).Fill((Tau1P4+Tau2P4).Phi(),w);
