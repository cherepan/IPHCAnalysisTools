...
void  ZTauTau::Configure(){
  ...
  PiOmega_minus=HConfig.GetTH1D(Name+"_PiOmega_minus","#omega_{#pi}(L)",40,-1.,1.,"#omega_{#pi} ","Events");
  PiOmega_plus=HConfig.GetTH1D(Name+"_PiOmega_plus","#omega_{#pi}(R)",40,-1.,1.,"#omega_{#pi}","Events");
 
  RhoOmega_minus=HConfig.GetTH1D(Name+"_RhoOmega_minus","#omega_{#rho}(L)",40,-1.,1.,"#omega_{#rho} ","Events");
  RhoOmega_plus=HConfig.GetTH1D(Name+"_RhoOmega_plus","#omega_{#rho}(R)",40,-1.,1.,"#omega_{#rho}","Events");
 

  A1Omega_minus=HConfig.GetTH1D(Name+"_A1Omega_minus","#omega_{a1}(L)",40,-1.,1.,"#omega_{a1} ","Events");
  A1Omega_plus=HConfig.GetTH1D(Name+"_A1Omega_plus","#omega_{a1}(R)",40,-1.,1.,"#omega_{a1}","Events");
...
void  ZTauTau::Store_ExtraDist(){
  ...
  Extradist1d.push_back(&PiOmega_minus);
  Extradist1d.push_back(&PiOmega_plus);
  Extradist1d.push_back(&RhoOmega_minus);
  Extradist1d.push_back(&RhoOmega_plus);


  Extradist1d.push_back(&A1Omega_minus);
  Extradist1d.push_back(&A1Omega_plus);
...
void  ZTauTau::doEvent()  { //  Method called on every event
  ...
  //   std::cout<<"============  id (This is SELECTED tautau2pipi)    "<< id <<std::endl<<std::endl;
  //   TLorentzVector Tau1Truth=Ntp->GetTruthTauLV(3,0);
  //   TLorentzVector Tau2Truth=Ntp->GetTruthTauLV(3,1);

  //   TLorentzVector TruthDecayFromTau1=Ntp->GetTruthTauProductLV(3,211,0);
  //   TLorentzVector TruthDecayFromTau2=Ntp->GetTruthTauProductLV(3,211,1);
  ...
  //   tauandprod_SinglePi.push_back(Tau1Truth); //  order matters! First should be tau
  //   tauandprod_SinglePi.push_back(TruthDecayFromTau1); // pion
  //   TauSinglePi.Configure(tauandprod_SinglePi,"pion");
  //   //    std::cout<<" Mass  "<< (Tau1Truth + Tau2Truth ).M()  <<std::endl;
  //   if(TauSinglePi.isConfigured()   && (Tau1Truth + Tau2Truth ).M() > 81. && (Tau1Truth + Tau2Truth ).M() < 101. ){
  //     PiOmega_minus.at(t).Fill(TauSinglePi.getOmegabar(),WeightMinus);
  //     PiOmega_plus.at(t).Fill(TauSinglePi.getOmegabar(),WeightPlus);


  //     std::cout<<"Run Number:     " <<Ntp->RunNumber() <<"  Event Number:     "<<Ntp->EventNumber()<<"  lumiblock  "<<Ntp-> LuminosityBlock() <<std::endl;
  //     std::cout<<"Tau_pi   four-momentum:     ";Tau1Truth.Print();
  //     std::cout<<"    pi   four-momentum:     ";TruthDecayFromTau1.Print();
  //     std::cout<<"    omega:                  "<< TauSinglePi.getOmegabar()<<std::endl;
  //     std::cout<<"    TauSpinnerWeight (+/-):                  "<<WeightPlus <<"  "<< WeightMinus<<std::endl<<std::endl<<std::endl<<std::endl;
  ...
  if(Ntp->CheckDecayID(2,4)){
    ...
    TLorentzVector 	  Tau1Truth=Ntp->GetTruthTauLV(2,0);
    TLorentzVector	  Tau2Truth=Ntp->GetTruthTauLV(4,1);
    std::vector<TLorentzVector> Pions2=Ntp->GetTruthPionsFromRho(1);


    TauPolInterface  TauSingleRho;
    vector<TLorentzVector> tauandprod_SingleRho;
    
    tauandprod_SingleRho.push_back(Tau2Truth); //  order matters! First should be tau
    tauandprod_SingleRho.push_back(Pions2.at(0)); // pi charged
    tauandprod_SingleRho.push_back(Pions2.at(1)); //  pi0
    TauSingleRho.Configure(tauandprod_SingleRho,"rho");
    if(TauSingleRho.isConfigured())
      {
	RhoOmega_minus.at(t).Fill(TauSingleRho.getOmegabar(),WeightMinus);
	RhoOmega_plus.at(t).Fill(TauSingleRho.getOmegabar(),WeightPlus);
    }
  }

  if(Ntp->CheckDecayID(2,5)){
    TLorentzVector Tau1Truth=Ntp->GetTruthTauLV(2,0);
    TLorentzVector Tau2Truth=Ntp->GetTruthTauLV(5,1);
    TLorentzVector TruthDecayFromTau1=Ntp->GetTruthTauProductLV(2,13,0);
    vector<TLorentzVector> Pions2=Ntp->GetTruthPionsFromA1(1);
    ...
    tauandprod_SingleA1.push_back(Tau2Truth);  //  order matters! First should be tau
    tauandprod_SingleA1.push_back(Pions2.at(0));    //  OS pi
    tauandprod_SingleA1.push_back(Pions2.at(1)); //  SS1 pi 
    tauandprod_SingleA1.push_back(Pions2.at(2));   //  SS2 pi  
	...
	A1Omega_minus.at(t).Fill(TauSingleA1.getOmegabar(),WeightMinus);
	A1Omega_plus.at(t).Fill(TauSingleA1.getOmegabar(),WeightPlus);
